...
export const Textarea = ({ className, variant = 'dark', ...props }: TextareaProps) => {
  // Use classname conditions instead of template strings so Tailwind can detect the classes
  return (
    <textarea
      className={`
        w-full px-4 py-3 rounded-lg transition-colors focus:ring-2 focus:border-primary-500 disabled:cursor-not-allowed text-base font-normal resize-vertical
        ${variant === 'dark'
